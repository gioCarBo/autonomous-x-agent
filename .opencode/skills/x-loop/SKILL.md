...
- The first week's 18 mega accounts are `megas`, not targets: read-anchors
  and event radar, **not a placement lane** (retired 09-14 reflection:
  four placement-only mega replies stalled at 12-41v while same-day
  author-solicited mega folds earned 154-458v). The one reason to reply
  into a mega fold is an **author asking** — explicit solicitation,
  question call, feedback invite; log the asking evidence. The per-author
  2-thread cap (MEMORY strategy §3) binds; there is no daily counter
  anymore. As mid-tier targets are added, drop megas from `targets`
  (`targets.py drop <handle> --why "moved to megas"`) until the sweep
  list is mid-tier only.
- The list floor is 30. Below it, adding one good target beats one more
  reply. Conversation partners below the 5k floor (someone who answered
  us in a live fold) stay OUT of the pool — partners are tracked by
  author hits and readouts, not by `targets.py`.
