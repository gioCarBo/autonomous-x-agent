...
class Poll(WriteVerbCase):
    TEXT = "Which reply shape earns answers?"
    OPTS = ["Question", "Silence"]

    def rules(self, set_ok=True, focus_ok=True, gate_ok=True):
        return (
            ("anchorOk", json.dumps({"ok": True, "stage": "focused"})),
            ("createPollButton", POS),
            ("HTMLInputElement.prototype", json.dumps(
                {"ok": set_ok, "stage": "option-not-registered"})),
            ("activeElement === inp", json.dumps(
                {"ok": focus_ok, "stage": "option-focus-failed"})),
            ("option-gate", json.dumps(
                {"ok": gate_ok,
                 "stage": "poll-gates-passed" if gate_ok else "btn-disabled"})),
            ("no-submit-btn", json.dumps({"ok": True, "stage": "gates-passed"})),
        )

    def test_bad_poll_before_any_tab(self):
        for opts, why in ((["one"], "single"), (["a", "b", "c"], "three"),
                          (["a", "  "], "blank")):
            err = self.assert_failed(
                self.run_verb(write.poll, self.TEXT, opts), "bad-poll", "poll")
            self.assertIn("exactly two non-empty", err["message"])
        self.assert_no_tab()

    def test_option_over_25_chars_fails_before_any_tab(self):
        err = self.assert_failed(
            self.run_verb(write.poll, self.TEXT, ["a" * 26, "b"]), "bad-poll", "poll")
        self.assertIn("25 chars", err["message"])
        self.assert_no_tab()

    def test_cap_reached_before_any_tab(self):
        self.caps(originals=5)
        err = self.assert_failed(
            self.run_verb(write.poll, self.TEXT, self.OPTS), "cap-reached", "poll")
        self.assertEqual(err["diagnostics"], {"url": None, "kind": "original"})
        self.assert_no_tab()

    def test_two_per_hour_rule(self):
        self.caps(post_times=[(self.now - timedelta(minutes=m)).isoformat(timespec="minutes")
                              for m in (12, 41)])
        err = self.assert_failed(
            self.run_verb(write.poll, self.TEXT, self.OPTS), "cap-reached", "poll")
    ...
    def test_toggle_click_only_in_check_mode(self):
        cdp = self.tab(*self.rules())
        ev = self.assert_ok(
            self.run_verb(write.poll, self.TEXT, self.OPTS, check_only=True), "poll")
        self.assertEqual(ev["checked"], True)
        self.assertEqual(ev["stage"], "gates-passed-not-submitted")
        self.assertEqual(len(cdp.mouse_events()), 2)  # createPollButton only
        self.assertEqual(self.state["caps"]["originals"], 0)  # not consumed
        self.assert_tab_closed()

    def test_option_setter_gate_fails_named_stage(self):
        cdp = self.tab(*self.rules(set_ok=False))
        err = self.assert_failed(
            self.run_verb(write.poll, self.TEXT, self.OPTS), "option-not-registered", "poll")
        self.assertEqual(len(cdp.mouse_events()), 2)  # toggle only, no submit
        self.assertEqual(self.bugs, [{"action": "poll",
                                      "signature": "poll:option-not-registered",
                                      "error_code": "option-not-registered"}])
        self.assert_tab_closed()

    def test_option_focus_gate_fails_named_stage(self):
        cdp = self.tab(*self.rules(focus_ok=False))
        self.assert_failed(self.run_verb(write.poll, self.TEXT, self.OPTS),
                           "option-focus-failed", "poll")
        self.assertEqual(len(cdp.mouse_events()), 2)
        self.assert_tab_closed()

    def test_submit_disabled_gate_fails_named_stage(self):
        cdp = self.tab(*self.rules(gate_ok=False))
        # gate_ok=False answers the POLL_OPTIONS probe with its own ok:false,
        # so the toggle click happened but _await_modal failed before submit.
        self.assert_failed(self.run_verb(write.poll, self.TEXT, self.OPTS),
                           "btn-disabled", "poll")
        self.assertEqual(len(cdp.mouse_events()), 2)
        self.assert_tab_closed()
