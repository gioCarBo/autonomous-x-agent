...
3. **Scoreboard clause (pivot 09-13 #5, live from 09-14):** the card
   ...
   dies.
4. **Card stats (live from 09-17):** the stat row prints needed/day for
   1,000 (from the scoreboard target) instead of profile visits — the
   visits number was publicly retired 09-15 as unauditable but the
   template kept printing it until 09-16 (miss named in that evening's
   postmortem). The pace stat is Zaid's stark-table rule: a flat day
   shows its own miss (needs 71, did +0).
5. **Pinned scoreboard, weekly:** on Mondays and on checkpoint days,
...
6. `x post --series metrics --media renders/metrics-YYYY-MM-DD.png --text "…"`
