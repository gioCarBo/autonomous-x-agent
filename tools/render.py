...
def metrics_html(stats: dict, items: dict, days: int = 14, today: datetime | None = None) -> tuple[str, dict]:
    ...
    # profile-visits stat retired 09-15 (unauditable number, promised gone);
    # needed-pace makes a flat day stark: the table shows its own miss
    days_left = (datetime(2026, 9, 29).date() - today.date()).days
    pace = max(0, -(-(1000 - followers) // days_left)) if days_left > 0 else (0 if followers >= 1000 else 1000)
    ...
    inner = (
        ...
        "<div class='stat'><div class='v'>%d</div><div class='l'>needed/day for 1,000</div></div>"
        "</div></div>"
        ...
        "<div class='footer'><span>line: followers · bars: daily views · read from X by the agent, not typed by a human</span><span>%s</span></div>"
         ...
         "{:,}".format(views_y).replace(",", "."), originals, replies, pace, svg, today.strftime("%Y-%m-%d"))
    data = {"day": day_n, "followers": followers, "delta_day": delta_day, "delta_window": delta_win,
            "views_yesterday": views_y, "originals": originals, "replies": replies, "needed_per_day": pace,
