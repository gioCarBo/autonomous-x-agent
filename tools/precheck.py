...
SKIP = 10
# Owner directive 2026-09-13 (aggression pivot): 10 replies/day is a floor.
# Until the floor is met every tick launches a hunting wake; once met, the
# normal signal-gating resumes. This is the anti-passivity enforcement —
# the launch week averaged ~2 replies/day because the timer could be quiet
# for 3h straight with a 21-target pool.
DAILY_REPLY_MIN = 10
...
def decide(now: datetime, last_llm: datetime | None, state: dict,
           ...
           series_due: list[str] | None = None,
           replies_today: int | None = None) -> tuple[bool, list[str]]:
    ...
    if replies_today is not None and replies_today < DAILY_REPLY_MIN:
        reasons.append("replymin:%d/%d" % (replies_today, DAILY_REPLY_MIN))
...
def replies_today(items_file: Path, now: datetime) -> int | None:
    """Replies published today (local date), from items.json. `None` = file
    unreadable: no signal either way (fail-quiet, the other signals decide)."""
    try:
        items = (json.loads(items_file.read_text()) or {}).get("items") or []
    ...
    for it in items:
        if it.get("kind") != "reply":
            continue
        p = _parse(str(it.get("published")), now)
        if p and p.date() == now.date():
            n += 1
    return n
...
def main(argv: list[str] | None = None, *, runner=run_verb, now: datetime | None = None,
    ...
    ap.add_argument("--max-silence-min", type=int, default=90)
    ...
    launch, reasons = decide(now, last_llm, state, notif, sweep,
                             ...
                             replies_today=replies_today(items_file, now))
