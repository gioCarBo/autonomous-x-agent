...
def main(argv: list[str] | None = None) -> None:
    ...
    pl = sub.add_parser("poll", help="publish a two-option poll "
                                     "(original cap; no media)")
    pl.add_argument("--text", required=True)
    pl.add_argument("--option", dest="options", nargs="+", required=True,
                    metavar="CHOICE",
                    help="exactly two choices, 1-25 chars each")
    pl.add_argument("--check", action="store_true",
                    help="run gates only, never submit")
    pl.add_argument("--series", help="series tag for items.json (take)")
    ...
    try:
        if args.verb == "status":
            ...
            write.poll(args.text, args.options, check_only=args.check,
                       series=args.series, model=args.model)
