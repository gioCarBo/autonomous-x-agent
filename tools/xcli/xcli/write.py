"""Write verbs: reply, post, poll, quote, like, repost, follow, bio, pin.
...
POLL_BUTTON_JS = """(() => {
  const d = [...document.querySelectorAll('div[role="dialog"]')]
    .find(d => d.querySelector('[data-testid^="tweetTextarea_"]'));
  const b = d && d.querySelector('[data-testid="createPollButton"]');
  if (!b) return null;
...
# The two choice fields are plain <input type=text> (no data-testid) that
# createPollButton reveals; they are matched positionally, values first.
POLL_OPTIONS_JS = """((a, b) => {
  const d = [...document.querySelectorAll('div[role="dialog"]')]
    .find(d => d.querySelector('[data-testid^="tweetTextarea_"]'));
  if (!d) return JSON.stringify({ok: false, stage: 'no-dialog'});
  const opts = [...d.querySelectorAll('input[type="text"]')]
    .map(i => i.value || '');
  const btn = d.querySelector('[data-testid="tweetButton"]');
  if (opts.length !== 2)
    return JSON.stringify({ok: false, stage: 'poll-fields',
      detail: opts.length + ' option fields (expected 2)'});
  if (opts[0] !== a || opts[1] !== b)
    return JSON.stringify({ok: false, stage: 'option-gate',
      detail: 'options read back as ' + JSON.stringify(opts)});
  if (!btn || btn.getAttribute('aria-disabled') === 'true')
    return JSON.stringify({ok: false, stage: 'btn-disabled',
      detail: 'submit disabled — X did not register the poll choices'});
  return JSON.stringify({ok: true, stage: 'poll-gates-passed'});
})"""

# bio recipe on a plain input: native value setter + tracker reset + events
POLL_SET_OPTION_JS = """((idx, text) => {
  const d = [...document.querySelectorAll('div[role="dialog"]')]
    .find(d => d.querySelector('[data-testid^="tweetTextarea_"]'));
  const inp = [...d.querySelectorAll('input[type="text"]')][idx];
  if (!inp) return JSON.stringify({ok: false, stage: 'no-option-field'});
  inp.focus();
  ...
  return JSON.stringify({ok: inp.value === text,
    stage: 'option-not-registered'});
})"""

POLL_FOCUS_OPTION_JS = """((idx) => {
  const d = [...document.querySelectorAll('div[role="dialog"]')]
    .find(d => d.querySelector('[data-testid^="tweetTextarea_"]'));
  const inp = [...d.querySelectorAll('input[type="text"]')][idx];
  if (!inp) return JSON.stringify({ok: false, stage: 'no-option-field'});
  inp.focus();
  return JSON.stringify({ok: document.activeElement === inp,
    stage: 'option-focus-failed'});
})"""


def poll(text: str, options: list[str], check_only: bool = False,
         series: str | None = None, model: str | None = None) -> None:
    """Publish a two-option poll (original-post cap; takes no media).

    Recipe scouted live 09-14 ~13:45 (nothing submitted, Escape after
    every pass): main text goes in FIRST while the composer editor is
    auto-focused (post recipe); the trusted click on createPollButton
    reveals two plain choice <input>s and moves focus to the first;
    option 0 takes the native value setter (bio recipe), option 1 takes
    JS focus + Input.insertText — the caret trap (issue #11) binds only
    the Lexical composer, not plain inputs. X disables the submit until
    both choices register in React state. The composer exposes exactly
    two choice fields, so v1 takes exactly two options, 1-25 chars each
    (X's choice limit)."""
    options = [o.strip() for o in (options or [])]
    if len(options) != 2 or not all(options):
        envelope.fail("poll", envelope.BUG, "bad-poll",
                      "a poll takes exactly two non-empty options "
                      f"(got {len(options)})",
                      diagnostics={"n": len(options)})
    for o in options:
        if len(o) > 25:
            envelope.fail("poll", envelope.BUG, "bad-poll",
                          f"option longer than 25 chars: {o!r} "
                          f"({len(o)} chars)",
                          diagnostics={"len": len(o)})
    _require_cap("poll", "original", None)
    cdp = session.open_tab("https://x.com/compose/post", "poll")
    try:
        session.assert_owner_session(cdp, "poll")
        cdp.goto("https://x.com/compose/post", settle=4.0)
        _await_modal(cdp, "poll", None, text, [DIALOG_FOCUS_JS])
        cdp.command("Input.insertText", {"text": text}, timeout=15)
        time.sleep(0.8)
        _click_center(cdp, POLL_BUTTON_JS, "poll", "no-poll-button", None, cdp)
        first = json.loads(cdp.evaluate(
            POLL_SET_OPTION_JS + f"(0, {json.dumps(options[0])})")
            or '{"ok":false}')
        if not first.get("ok"):
            _fail_gate("poll", None, first, text, cdp)
        focus = json.loads(cdp.evaluate(POLL_FOCUS_OPTION_JS + "(1)")
                           or '{"ok":false}')
        if not focus.get("ok"):
            _fail_gate("poll", None, focus, text, cdp)
        cdp.command("Input.insertText", {"text": options[1]}, timeout=15)
        time.sleep(0.8)
        _await_modal(cdp, "poll", None, text,
                     [POLL_OPTIONS_JS
                      + f"({json.dumps(options[0])}, {json.dumps(options[1])})"])
        _submit_and_verify(cdp, "poll", None, text, check_only,
                           cap_kind="original",
                           tag={"kind": "original", "series": series,
                                "model": model})
    finally:
        cdp.close()
